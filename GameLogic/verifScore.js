...
export function verifScore(userId, io) {
  ...
  gameLogic.User[userId].score = Math.floor(
    (currentTime - gameLogic.User[userId].startGame) / 15,
  );
  io.to(userId).emit("ScoreLost", gameLogic.User[userId].score);
  console.log("score de verif", gameLogic.User[userId].score);
  addscore(
    gameLogic.User[userId].name,
    gameLogic.User[userId].score,
    gameLogic.User[userId].skin,
  );
}
